...
export async function listClusterThemes(clusterId: string): Promise<ClusterTheme[]> {
...
export async function archiveClusterTheme(themeId: string): Promise<void> {
  const { error } = await supabase
    .from('cluster_themes')
