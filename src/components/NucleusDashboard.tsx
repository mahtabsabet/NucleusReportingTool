...
import {
  ...
  setBannerImage,
  getNucleusNotes,
  setNucleusNotes } from
'../data/store';
const defaultRolesForType: Record<string, string[]> = {
...
export function NucleusDashboard() {
  ...
  const [activities, setActivities] = useState<Activity[]>(
    initialActivities.filter((a) => a.nucleusId === id)
  );
  const [showAddActivity, setShowAddActivity] = useState(false);
  const [newActivityType, setNewActivityType] =
  useState<string>('children-class');
  ...
  // Nucleus notes state
  const [notes, setNotes] = useState(() => getNucleusNotes(id!));
  const [notesSaved, setNotesSaved] = useState(false);
  const handleSaveNotes = () => {
    setNucleusNotes(id!, notes);
    setNotesSaved(true);
    setTimeout(() => setNotesSaved(false), 2000);
  };
  if (!nucleus) {
    return <div>Nucleus not found</div>;
  }
  // Get people from store for the third panel
  const people = getPeopleForNucleus(id!);
  ...
  const handleCreateActivity = () => {
    ...
    const roles = defaultRolesForType[newActivityType] || ['participants'];
      ...
      <div className="max-w-[1600px] mx-auto p-4 sm:p-8 space-y-6 sm:space-y-8">
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody>
                  ...
                  <tr
                    key={activity.id}
                    onClick={() =>
                    navigate(`/nucleus/${id}/activity/${activity.id}`)
                    }
        ...
        <div className="bg-white/95 backdrop-blur-sm border border-gray-200/80 rounded-2xl p-7 shadow-sm hover:shadow-md transition-shadow duration-300">
          <div className="mb-8">
            ...
            <p className="text-sm text-gray-500">
              Educational progress and capacities of individuals in this
              nucleus.
            </p>
          </div>
