...
export function Timeline({ clusterId }: TimelineProps) {
  ...
  const loadData = () => {
    fetchTimelineCycles().then(setCycles).catch(() => {});
  ...
  const adjustCycleBoundary = (cycleId: string, days: number) => {
    ...
    const updatedCycles = cycles.map((c, i) => {
      if (i === cycleIndex) return { ...c, endDate: newEnd };
      if (i === cycleIndex + 1) {
        const nextStart = new Date(newEnd);
        nextStart.setDate(nextStart.getDate() + 1);
        return { ...c, startDate: nextStart };
      }
      return c;
    });
    ...
    if (cycleIndex + 1 < cycles.length) {
      const nextStart = new Date(newEnd);
      nextStart.setDate(nextStart.getDate() + 1);
      updateCycleBoundary(cycles[cycleIndex + 1].id, nextStart, undefined).catch(console.error);
    }
  };

  const years = Array.from(new Set(cycles.map(c => c.startDate.getFullYear()))).sort();
  ...
  const getDatePercent = (date: Date, rangeStart: Date, rangeEnd: Date) => {
    ...
    if (total <= 0) return 0;
    return Math.max(0, Math.min(100, ((date.getTime() - rangeStart.getTime()) / total) * 100));
  };
  ...
  const renderEvents = (start: Date, end: Date) => {
    ...
    for (const evt of sorted) {
      ...
      if (!placed) { rows.push({ end: evtEnd }); eventRows.push(rows.length - 1); }
    }

    return (
      <div className="absolute left-0 right-0 bottom-[55%]" style={{ height: `${rows.length * 14 + 4}px` }}>
        {sorted.map((e, i) => {
        ...
        <div className="min-w-max w-full flex items-center h-full relative px-8">
          ...
          {cycles.length === 0 && (
            <div className="text-sm text-gray-400 italic mx-auto">
              No timeline cycles yet. Add cycles via the Supabase dashboard.
            </div>
          )}
          ...
          {zoomLevel === 'year' && selectedYear && cycles
            ...
            .map((cycle) => (
              <div key={cycle.id} className="flex-1 min-w-[250px] relative h-full flex flex-col justify-center group">
                ...
                <div className="absolute top-1/2 mt-4 left-0 -translate-x-1/2 text-xs font-medium text-gray-500">{formatDate(cycle.startDate)}</div>
                ...
                <div className="absolute top-1/2 mt-4 right-0 translate-x-[-50%] flex flex-col items-center gap-1 z-20">
                  <div className="text-xs font-medium text-gray-500">{formatDate(cycle.endDate)}</div>
          ...
          {zoomLevel === 'cycle' && selectedCycle && (() => {
            ...
            while (curr <= selectedCycle.endDate) {
              ...
              const actualEnd = monthEnd > selectedCycle.endDate ? selectedCycle.endDate : monthEnd;
              months.push({ start: new Date(curr), end: new Date(actualEnd), label: formatMonth(curr) });
            ...
            return months.map((month, index) => (
              <div key={index} className="flex-1 min-w-[200px] relative h-full flex flex-col justify-center group cursor-pointer" onClick={() => { setSelectedMonth(month); setZoomLevel('month'); }}>
                <div className="absolute top-1/2 -translate-y-1/2 left-0 w-0.5 h-4 bg-gray-400" />
                <div className="absolute top-1/2 mt-3 left-0 -translate-x-1/2 text-xs font-medium text-gray-500">{formatDate(month.start)}</div>
                {index === months.length - 1 && <div className="absolute top-1/2 mt-3 right-0 translate-x-1/2 text-xs font-medium text-gray-500">{formatDate(month.end)}</div>}
          ...
          {zoomLevel === 'month' && selectedMonth && (() => {
            ...
            while (curr <= selectedMonth.end) {
              ...
              const actualEnd = weekEnd > selectedMonth.end ? selectedMonth.end : weekEnd;
              weeks.push({ start: new Date(curr), end: new Date(actualEnd), label: `Week ${weekNum}` });
            ...
            return weeks.map((week, index) => (
              <div key={index} className="flex-1 min-w-[150px] relative h-full flex flex-col justify-center">
                ...
                <div className="absolute top-1/2 mt-2 left-0 -translate-x-1/2 text-[10px] font-medium text-gray-400">{formatDate(week.start)}</div>
                {index === weeks.length - 1 && <div className="absolute top-1/2 mt-2 right-0 translate-x-1/2 text-[10px] font-medium text-gray-400">{formatDate(week.end)}</div>}
